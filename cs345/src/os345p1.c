...
extern int histSize;
extern char hist[MAX_HIST][MAX_MESSAGE_SIZE];
...
int P1_shellTask(int argc_, char* argv_[])
{
	...
	while (1)
	{
		...
		SWAP										// do context switch
		{
			...
			char *p = myArgv[0];
			for ( ; *p; ++p) // makes command all lowercase
			{
				if(*p == ' ') break;
				*p = tolower(*p);
			}
...
int P1_help(int argc, char* argv[])
{
	...
	if(argc == 1){
		...
		for (i = 0; i < NUM_COMMANDS; i++)
		{
			if (strstr(argv[1],(commands[i]->command)) != NULL ||
					strstr(argv[1],(commands[i]->shortcut)) != NULL){
				printf("\n   %s (%s): %s", commands[i]->command, commands[i]->shortcut, commands[i]->description);
				return 0;
			}
		}
		printf("\nNo command found!");
...
int P1_add(int argc, char* argv[])
{
	...
	else
	{
		...
		while(i < argc)
		{
			int temp = (int)strtol(argv[i], NULL, 0);
			total += temp;
			i++;
		}
		printf("\n%d", total);
...
int P1_history(int argc, char* argv[])
{
	int i;
...
Command** P1_init()
{
	...
	commands[i++] = newCommand("add", "add", P1_add, "Computer parameters added together");
	commands[i++] = newCommand("args", "args", P1_args, "Echo arguments");
	commands[i++] = newCommand("date/time", "dt", P1_date_time, "Print out current date and time");
	commands[i++] = newCommand("loop", "loop", P1_loop, "Execute an infinite loop");
	commands[i++] = newCommand("history", "hist", P1_history, "Show previous histroy");
	...
	commands[i++] = newCommand("queuetest", "qt", queueTest, "Queue Test");
	commands[i++] = newCommand("semaphoretest", "st", semaphoreTest, "Semaphore Test");
	...
	commands[i++] = newCommand("deltaclocktest", "dct", P3_tdc, "List deltaclock entries (test)");
	...
	commands[i++] = newCommand("frametest", "ft", P4_getFrameTest, "Run frame test");

	// P5: Scheduling
	commands[i++] = newCommand("project5", "p5", P5_project5, "P5: Scheduling");
	//	commands[i++] = newCommand("stress1", "t1", P5_stress1, "ATM stress test1");
	commands[i++] = newCommand("calc", "rt", recalculateTimeCommand, "Recalculate Time Test");
