...
public class Matrix {
	...
	public float[][] toFloatArray(){
		float[][] results = new float[rows()][cols()];
		for(int row = 0; row < rows(); row++){
			for(int col = BackProp.VOWEL? 1:0; col < cols(); col++){
				results[row][col] = (float)get(row, col);
			}
		}
		return results;
	}
	...
	public float[][] getTargets(){
		float[][] targets = new float[rows()][valueCount(0)];
		for(int row = 0; row < rows(); row++){
			targets[row][(int)row(row)[0]] = 1;
		}
		return targets;
	}
